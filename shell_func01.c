...
/**
 * process_command - manages the process a command goes through to get executed
...
int process_command(char **args)
{
	...
	while (*args != NULL && prev_eval != EXIT_SHELL)
	{
		...
		if (str_comp(*args_ptr, "||", MATCH) == TRUE)
		{
			*args_ptr = NULL;
			args_ptr++;
			next_op = '|';
...
int built_ins(char **args)
{
	...
	i = alias_handler(args, FALSE);
	if (i == DO_EXECVE || i == SKIP_FORK)
		return (i);

	if (str_comp("exit", *args, MATCH) == TRUE && args[1] != NULL)
	{
		status = _stoi(args[1]);
		if (status < 0)
		{
			status = 2;
			print_error_msg(args[0], args[1]);
			return (SKIP_FORK);
		}
	}
	if (str_comp("exit", *args, MATCH) == TRUE)
		return (EXIT_SHELL);
	else if (str_comp("setenv", *args, MATCH) == TRUE && args[1] != NULL)
		return (_setenv(args[1], args[2]));
	else if (str_comp("unsetenv", *args, MATCH) == TRUE
		 && args[1] != NULL)
		return (_unsetenv(args[1]));
	else if (str_comp("cd", *args, MATCH) == TRUE)
		return (_chngdir(args[1]));
	else if (str_comp("env", *args, MATCH) == TRUE)
		return (print_env());
...
char *check_command(char **args)
{
	...
	path_var = create_array(path_str, ':', NULL);
...
int execute_command(char **args)
{
	...
	if (str_comp("false", *args, MATCH) == TRUE)
		status = 1;
